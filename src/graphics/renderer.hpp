...
public:
    ...
    void draw(const RenderQueue &queue);                        // RenderQueue의 draw command를 실제로 그리기
    bool windowShouldClose() const { return should_close_; };   // 창이 닫혔는지 질의
...
private:
    ...
    GLFWwindow *window_ = nullptr; // GLFW 윈도우/컨텍스트

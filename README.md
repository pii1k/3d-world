2025-12-25 New Features
카메라 구조 변경
이전 구조
- 타겟 위치 기준으로 카메라 위치 계산
- 그 위치에서 타겟을 바라보게 함(lookAt)
-> 카메라가 대상 물체를 바라보도록 강제되어 있어서 물체가 움직이면 카메라 방향이 자동으로 바뀜

변경된 구조
- yaw/pitch로 카메라 좌표계 먼저 생성
- 그 좌표계의 forward 방향을 기준으로, 타겟 뒤로 distance 만큼 물러난 위치에 카메라를 두게 함
-> 카메라의 방향은 고정 상태값(yaw/pitch)이고 물체가 움직이면 카메라의 위치만 바뀜

---

2025-12-24 일단 성능은 잠시 포기한다.
처음부터 잡으면서 가려니까 답이 없다.
나중에 진짜 필요하면 그때 최적화 하는걸로..
